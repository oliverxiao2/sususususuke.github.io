var Figure = function(parent, type, options){
	...
	fgLayer.style.height = '100%';
	fgLayer.style.position = 'absolute';
	fgLayer.style.zIndex = 100;
	fgLayer.setAttribute('data-element-type', 'fgLayer');
	wrapper.appendChild(fgLayer);

	var floatCursor = document.createElement('div');
	this.floatCursor = floatCursor;
	floatCursor.parent = this;
	floatCursor.style.width = '2px';
	floatCursor.style.height = '100%';
	floatCursor.style.position = 'absolute';
	floatCursor.style.top = this.setting.axisTopHeight;
	floatCursor.style.left = 10;
	floatCursor.style.backgroundColor = 'red';
	fgLayer.appendChild(floatCursor);

	fgLayer.addEventListener('mousedown', mousedownHandler);
	fgLayer.addEventListener('mousemove', mousemoveHandler);
	fgLayer.addEventListener('mouseup', mouseupHandler);
	fgLayer.addEventListener('mousewheel', mousewheelHandler);
	fgLayer.addEventListener('contextmenu', contextmenuHandler);
	// ============================================
	...
	function mousedownHandler (e) {
		...
		const where = me.whereFocused(x, y); console.log(where);
		...
		else if(_type === 'edgeOfAxisLeft'){
			me.actionTarget = where.target;
			me.setting.action = 'rebuild';
		}
	...
	function mouseupHandler (e) {
		...
		me.wrapper.style.cursor = 'pointer';
	};
	...
	function mousemoveHandler (e) {
		...
		if (me.state === 'pan' && me.actionTargetType === 'group') {
		...
		else if (me.state === 'idle' && e.altKey) {
			console.log(e);
			if (!where) return false;

			if (where.type === 'group' || where.type === 'bitGroup') {
				me.floatCursor.style.left = e.offsetX;
			}
		}

		else if (me.state === 'idle') {
			var xPoint = parseInt(e.offsetX);
			var moveLength = 0;
				if (where && where.type === 'edgeOfAxisLeft') {
				me.wrapper.style.cursor = 'col-resize';
				if(e.which === 1){
					for (var plot of me.plotCollection){
				  		plot.width = xPoint - plot.setting.xAxis.setting.xStart;
				  		moveLength = xPoint - plot.setting.xAxis.setting.xEnd;
					}
				}
				//return false;
			}
			else {
				me.wrapper.style.cursor = 'auto';
				if(e.which === 1){
					for (var plot of me.plotCollection){
				  		plot.width = xPoint - plot.setting.xAxis.setting.xStart;
				  		moveLength = xPoint - plot.setting.xAxis.setting.xEnd;
			...
			me.setting.legendTableWidth -= moveLength;
			fig1.figure.draw();
		}
	};
...
Figure.prototype.whereFocused = function (x, y) {
	if (x > (this.width - this.setting.legendTableWidth)) {
		...
		for (const [i, axisLeft] of this.axisCollection.axisLeftCollection.entries()) {
			if (i === 0 && x > axisLeft.setting.xStart - 2) return {type: 'edgeOfAxisLeft'};

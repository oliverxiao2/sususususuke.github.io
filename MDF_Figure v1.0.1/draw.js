Figure.prototype.draw = function(options){
	...
	function layout (fig, channels, options) {
		const output = {
			groups: [],
			bitGroup: null,
		};
		...
		// 按照groupId和isBit分拣好channel
		for (const channel of channels) {
			const isBit = channel.isBit;
			const groupId = channel.group;
			if (isBit) bitGroup.push(channel);
			else {
		...
		if (options.mode === 'multiY') {
			for (const group of groups) {

				for (const channel of group.channels) {
					if (channel.setting.yAxis) {

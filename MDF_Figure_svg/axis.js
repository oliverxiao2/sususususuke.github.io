Figure.prototype.addAxis = function(options){
	...
	svgDom.setAttribute('data-figure-element', 'axis');
	...
	canvas.draw = function (){
		...
		if (setting.visible) {
			...
			if (setting.showSpine) {
				...
				const _x = d3.scaleLinear().domain(setting.domain).range([setting.yStart, setting.yEnd]);console.log([setting.yStart, setting.yEnd]);
				const _axis = d3.axisLeft(_x);
				d3.select(svgDom).append('g')
					.attr('transform', 'translate('+setting.xStart+', 0)')
					.call(_axis);
			}

...
chart.prototype.init = function(container, viewmode=0){
  ...
  if (viewmode === 0){
    ...
    */
    ...
    this.wrapper.style.border = '1px solid #666';
    ...
    this.contextmenuElement.innerHTML = '<ul><li>item1</li><li>item2</li></ul>'
...
line.prototype.draw2 = function(forceRepainting = false, viewmode=0){
  console.time('draw');
  ...
  let channelIndex = -1, groupIndex = -1, theFileCanvas;
  ...
  for (const [fileIndex, filename] of filenames.entries()){
    ...
    if (!file.canvas) {
      ...
      theFileCanvas = file.canvas;
      theFileCanvas.width = this.width;
      theFileCanvas.height= this.height;
      theFileCanvas.parentFile = file;
      this.container.appendChild(theFileCanvas);
    } else {
      theFileCanvas = file.canvas;
    }
    ...
    if (file.dataGroup.length > 0){
      ...
      for (const group of file.dataGroup){
        ...
        if (file.repaint || canvas.repaint || forceRepainting){
